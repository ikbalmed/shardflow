...
- Built a Kafka-inspired streaming backend in Python and FastAPI with segmented log storage and offset-based consumption.
- Implemented consumer group coordination with registration, heartbeats, deterministic assignment, and rebalance behavior.
- Added a local cluster simulation with broker registration, heartbeats, leader/follower metadata, replica lag tracking, and failover.
- Exposed operational APIs and a CLI for common broker and topic workflows.
- Designed the system to be understandable, testable, and runnable on a single machine without external infrastructure.
